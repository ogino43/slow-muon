                     ...
                     */
                     ...
                     if (xpeakdif > 25 || linging > 0.5) {
                         if (io < former) {
                           ...
                           //  hq16->Fill((xpeak1[j] * rebin - 3790)/1000, ypeak1[j]);
                         ...
                         if (io > later) {
                          ...
                          //   hq17->Fill((xpeak1[j] * rebin - 3790)/1000, ypeak1[j]);
                         }
                     }
                     else;
...
for (int i = 1; i < nbins; i++) {
    hq14->SetBinContent(i, (hq14->GetBinContent(i)/1084)*1051);
}

int nbins = hq13->GetNbinsX();
for (int i = 1; i < nbins; i++) {
    hq13->SetBinContent(i, hq13->GetBinContent(i));
}

//hq13->GetYaxis()->SetRangeUser(0, 9);
//hq13->GetXaxis()->SetRangeUser(-0.5, 6);
...
int nbins = hq13->GetNbinsX();
double x, y, z, e;
std::ofstream file("C:\\Users\\NagataLabo\\Documents\\muon\\2024Aexp\\toukei\\output2.txt");
if (!file) { // ファイルが開けなかった場合のエラーチェック
    std::cerr << "cant open file" << std::endl;
    return 1;
}
for (int i = 1; i < nbins; i++) {
    x1 = hq13->GetBinCenter(i);
    x2 = hq14->GetBinCenter(i);
    y1 = hq13->GetBinContent(i);
    y2 = hq14->GetBinContent(i);
    file << x1 << " " << y1 <<" " << x2 << " " << y2 << std::endl; // 数字を書き出し
...
hq14->Draw("same");
/*
TLine* line = new TLine(5210-3790, 0, 5210 - 3790, 9000);
line->SetLineStyle(2);  // 点線スタイル（1: 実線、2: 点線、3: ダッシュなど）
line->SetLineColor(kBlack);  // 線の色を赤に設定
line->Draw("same");  // ヒストグラムと同じキャンバスに描画
TLine* line2 = new TLine(4610 - 3790, 0, 4610 - 3790, 9000);
line2->SetLineStyle(2);  // 点線スタイル（1: 実線、2: 点線、3: ダッシュなど）
line2->SetLineColor(kBlack);  // 線の色を赤に設定
line2->Draw("same");  // ヒストグラムと同じキャンバスに描画
*/
/*
hq15->SetLineColor(3);
