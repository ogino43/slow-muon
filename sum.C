...
TChain *ch1= new TChain("tree",""); // treeNameという名のTTreeのchain
 ...
 TH1D* hq14 = new TH1D("", "", 2000, 0, 20000);
 TH1D* hq15 = new TH1D("", "", 2000, 0, 20000);
...
c1->cd(1)->SetLogy();
hq4->GetYaxis()->SetTickLength(-0.02);
hq4->Draw();
gp->Draw("L");
...
hq13->GetYaxis()->SetRangeUser(0, 350 * pow(10, -6.0));
hq13->GetXaxis()->SetRangeUser(3600, 10000);

for (int i = 1;i <= hq13->GetNbinsX();i++) {
    hq13->SetBinContent(i, hq13->GetBinContent(i) / (6*pow(10,6.0)));
}

hq13->SetTitle(Form("%s ,%d < y < %d", buffer, minheight, maxheight));
hq13->SetLineColor(kRed);
hq13->Draw();

/*
TLine* line = new TLine(4890, 0, 4890, 280 * pow(10, -6.0));
line->SetLineStyle(2);  // 点線スタイル（1: 実線、2: 点線、3: ダッシュなど）
line->SetLineColor(kBlack);  // 線の色を赤に設定
line->Draw("same");  // ヒストグラムと同じキャンバスに描画
TLine* line2 = new TLine(5490, 0, 5490, 280 * pow(10, -6.0));
