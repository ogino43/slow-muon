...
TChain *ch1= new TChain("tree",""); // treeNameという名のTTreeのchain
 ...
 TH1D* hq20 = new TH1D("", "", 1000, 0, 20000);//
             ...
             */
                 ...
                 if (j >= 1) {
                     ...
                     */
                     ...
                     if (xpeakdif > 25 || 0.3< linging) {

                         hq4->Fill(ypeak1[j]);
                         hq13->Fill(xpeak1[j] * rebin);
                         hq16->Fill(xpeak1[j] * rebin, ypeak1[j]);
                         
                         if ((xpeak1[j] < 2440 || xpeak1[j] > 2700) || (xpeak1[j] > 2500 && xpeak1[j] < 2600)) {
...
double x, y, z, e;
std::ofstream file("C:\\Users\\NagataLabo\\Documents\\muon\\2024Aexp\\toukei\\output.txt");
...
for (int i = 1; i < nbins; i++) {
 ...
 //   y = (hq13->GetBinContent(i) - fitFunc->Eval(x)) / (fitFunc->Eval(x));
    z = fitFunc->Eval(x);
    y = hq13->GetBinContent(i) - z;

    file << x << " " << y << " " << z << std::endl; // 数字を書き出し
...
gt1->GetXaxis()->SetRangeUser(4500, 8000);
gt1->GetYaxis()->SetRangeUser(-200,200 );
gt1->Draw("AP");
