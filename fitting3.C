...
TF1* gaus1 = new TF1("gaus1", "gaus", minfit, maxfit);
TF1* fitFunc2 = new TF1("fitFunc2", "fitFunc+gaus1", minfit, maxfit);
fitFunc2->SetNpx(2000);
fitFunc2->SetParameters(15900, 2197, 0,100,5250,1000);
//fitFunc->FixParameter(1, 2197);
//fitFunc2->FixParameter(2, 0);
hq13->Fit("fitFunc2", "", "", 4700, 6000);
...
TGraph* gt1 = new TGraph();
int nbins = hq13->GetNbinsX();
double x, y, e;
for (int i = 1; i < nbins; i++) {
    x = hq13->GetBinCenter(i);
    y = (hq13->GetBinContent(i) - fitFunc2->Eval(x)) ;
    cout << x  << endl;
    cout << y << endl;
...
gt1->GetXaxis()->SetRangeUser(4000, 8000);
gt1->GetYaxis()->SetRangeUser(0,500);
gt1->Draw("AP");
